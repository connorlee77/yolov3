...
def kitti(setname):
    ...
    frames = glob.glob(os.path.join('/home/fremont/ford/cars/cars_1000', '*.jpg'))

    column_names = ['image', 'class', 'x1', 'y1', 'x2', 'y2']
    class_names = set(load_classes('data/coco.names'))

    classes = os.listdir('stanford_cars_cam')
    classes.sort()
    for c in classes:
    ...
    for frame in tqdm.tqdm(frames):        
        ...
        for name in classes:
            cam_mat = loadmat(os.path.join('stanford_cars_cam', name, matname))['cam']
            diff_mat = loadmat(os.path.join('stanford_cars_diff', name, matname))['cam']
            grad_mat = loadmat(os.path.join('stanford_cars_gradient', name, matname))['cam']
            temp_mat = loadmat(os.path.join('stanford_cars_tempcam', name, matname))['cam']
    ...
    df.to_csv('stanford_cars.csv'.format(setname))
