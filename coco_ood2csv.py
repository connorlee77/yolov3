...
def kitti(setname):
    ...
    classes = os.listdir('coco_val_{}_out_cam'.format(setname))
    classes.sort()
    for c in classes:
    ...
    for frame in tqdm.tqdm(frames): 
        ...
        for label in labels:
            ...
            for name in classes:
                cam_mat = loadmat(os.path.join('coco_val_{}_out_cam'.format(setname), name, matname))['cam']
                # diff_mat = loadmat(os.path.join('kitti_{}_out_diff'.format(setname), name, matname))['cam']
                grad_mat = loadmat(os.path.join('coco_val_{}_out_gradient'.format(setname), name, matname))['cam']
    ...
    df.to_csv('coco_val_{}.csv'.format(setname))
